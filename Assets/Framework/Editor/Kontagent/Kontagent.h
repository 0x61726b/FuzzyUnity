...
extern NSString * const KT_HTTP_SECURE_PROTOCOL;
extern NSString * const KT_HTTP_PROTOCOL;
extern NSString * const KT_PRODUCTION_API_URL_PREFIX;
extern NSString * const KT_TEST_API_URL_PREFIX;
extern NSString * const KT_URL_SUFFIX;
...
/**
 * \brief   Causes all communication with Kontagent servers to go through an HTTPS connect
 * @param   enabled YES to connect over HTTPS, NO to use HTTP
 */
+ (void) setSecureHttpConnection:(BOOL)enabled;

/**
 * \brief   Determines whether the SDK is using HTTPS or HTTP to connect to Kontagent servers
 * @return YES if the connection is using HTTPS, NO if it's using HTTP
 */
